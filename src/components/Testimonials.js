...
const Testimonials = () => {
    const testimonialTemplate = (testimonial, key) => <div className="px-8 bg-white shadow-lg rounded-lg py-10 bg-opacity-25 testimonial-card" key={key}>
        ...
        <div>
            <h2 className="text-gray-800 text-2xl font-semibold">
                <svg xmlns="http://www.w3.org/2000/svg" className="inline align-baseline" width="20" height="20" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#2c3e50" fill="none" strokeLinecap="round" strokeLinejoin="round">
        ...
        [showRemaining, setShow] = React.useState(false),
        listTestimonials = porfolioData.testimonials.slice(0, 3).map((testimonial, index) => testimonialTemplate(testimonial, index)),
        remainingTestimonials = porfolioData.testimonials.slice(3).map((testimonial, index) => testimonialTemplate(testimonial, index));
    return (
